...
def main():
    ...
    if args.option == 'receiver':
        ...
        check_call(cmd, stdout=open(path.join(args.aurora_save_dir, "vivace_latency_receiver_stdout.log"), 'w', 1),
                        stderr=open(path.join(args.aurora_save_dir, "vivace_latency_receiver_stderr.log"), 'w', 1))
        return

    if args.option == 'sender':
        os.environ['LD_LIBRARY_PATH'] = path.join(send_dir)
        cmd = [send_src, args.ip, args.port, "1"]
        import time
        time.sleep(4.5)
        if not os.path.exists(args.aurora_save_dir):
            os.makedirs(args.aurora_save_dir)
        check_call(cmd, stdout=open(path.join(args.aurora_save_dir, "vivace_latency_sender_stdout.log"), 'w', 1),
                        stderr=open(path.join(args.aurora_save_dir, "vivace_latency_sender_stderr.log"), 'w', 1))
        return
