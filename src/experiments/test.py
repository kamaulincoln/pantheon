...
class Test(object):
    def __init__(self, args, run_id, cc):
        ...
        self.pyprogram = args.pyprogram  # used to call Aurora later
    ...
    def run_first_side(self, tun_id, send_manager, recv_manager,
        ...
        if self.run_first == 'receiver':
            ...
            second_cmd = 'tunnel %s python %s sender %s %s --model-path %s --aurora-save-dir %s --pyprogram %s\n' % (
                tun_id, second_src, recv_pri_ip, port, self.model_path, self.aurora_save_dir, self.pyprogram)
